class Board:
    ...
    def score2(self,side):
        ...
        if len(self.moves[self.opponent(side)])==0:
            return 1000
        return math.log(float(len(self.moves[side]))/float(len(self.moves[self.opponent(side)]))) #Taking the log a million times takes half a second. This part of the program is fast enough.
...
##            return move_list #If the board has no towers, the player must make a reserve move on empty space.
        better_moves=[]
        for move in move_list:
            if not(move[0:2]=='rr' and self.mainboard[self.strtonum(move[2:4])][-1:]!=self.opponent(self.side)):#If the move is a reserve move that does not capture an enemy piece, the move is a bad move and should not be considered.
